...
apply(
  ...
  function(x){
    ...
    if (isTRUE(x["cv_12"] < max_cv)) {
      return(mean(x["conc_1"], x["conc_2"]))
    } else {
      sel <- which(x == min(c(x["cv_12"], x["cv_13"], x["cv_23"]), na.rm = TRUE))
      if (length(sel) == 0) {return(NA)}
      if (sel == 1) {return(mean(x["conc_1"], x["conc_2"]))}
      if (sel == 2) {return(mean(x["conc_1"], x["conc_3"]))}
      if (sel == 3) {return(mean(x["conc_2"], x["conc_3"]))} else {return(NA)}
    }
  }
)
...
```{r}
LEEF_2_plot_tocs_per_bottle_per_timestamp(db = db, c("TC", "TOC"))
LEEF_2_plot_tocs_per_bottle_per_timestamp(db = db, c("IC"))
LEEF_2_plot_tocs_per_bottle_per_timestamp(db = db, c("TN"))
```
